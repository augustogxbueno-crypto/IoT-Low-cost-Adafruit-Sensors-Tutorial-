...
const __FlashStringHelper* LTR329Sensor::name() const { return F("LTR329"); }
const __FlashStringHelper* LTR329Sensor::sensorType() const { return F("photodiode"); }

ChannelInfo LTR329Sensor::channel(uint8_t idx) const {
  return { Utils::flashStr(CH_NAMES, idx), Utils::flashStr(CH_COLORS, idx) };
}

bool LTR329Sensor::readChannels(float* out) {
...
const __FlashStringHelper* LTR329Sensor::gainOptionLabel(uint8_t idx) const {
  return Utils::flashStr(GAIN_NAMES, idx);
}

void LTR329Sensor::setGainIndex(uint8_t idx) {
  gainIdx_ = idx;
  sensor_.setGain(GAIN_VALUES[gainIdx_]);
}

float LTR329Sensor::integrationTimeMs() const { return TIME_MS[timeIdx_]; }

const __FlashStringHelper* LTR329Sensor::timePresetLabel(uint8_t idx) const {
  return Utils::flashStr(TIME_NAMES, idx);
}

void LTR329Sensor::setTimePreset(uint8_t idx) {
  timeIdx_ = idx;
  sensor_.setIntegrationTime(TIME_VALUES[timeIdx_]);
}

ExtraParamInfo LTR329Sensor::extraParam(uint8_t) const {
  return { "measRate", F("Measurement Rate"), (const __FlashStringHelper* const*)RATE_NAMES, 6, 0, 0 };
}

void LTR329Sensor::setExtraParam(uint8_t, int value) {
